...
export function useMarketIntelligence(
  ...
  const intelligence = useIntelligenceStore((state) =>
    marketAddress ? state.getIntelligence(marketAddress) : undefined,
  );
  ...
  const isLoading = marketAddress ? loadingMarkets.has(marketAddress) : false;
