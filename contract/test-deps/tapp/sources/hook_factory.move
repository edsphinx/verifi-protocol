module tapp::hook_factory {
    ...
    const HOOK_PREDICTION: u8 = 4;
    const YOUR_HOOK: u8 = 5;
    ...
    public(package) fun create_pool(
        ...
        if (hook_type == HOOK_PREDICTION) {
            let seed = vector[hook_type];
            seed.append(VeriFiPublisher::tapp_prediction_hook::pool_seed(assets, fee));
            let cref = create_named_object(vault, seed);
            let pool_signer = &generate_signer(&cref);
            let pool_addr = address_of(pool_signer);
            VeriFiPublisher::tapp_prediction_hook::create_pool(pool_signer, assets, fee, creator);
            let pool_meta = new_pool_meta(pool_addr, hook_type, assets);
            move_to(pool_signer, pool_meta);
            return cref
        };
    ...
    public(package) fun add_liquidity(
        ...
        if (pool_meta.hook_type == HOOK_PREDICTION) {
            let (amounts, mint_position) =
                VeriFiPublisher::tapp_prediction_hook::add_liquidity(
                    pool_signer, position_idx, stream, creator
                );
            let assets = pool_meta.assets;
            let txs =
    ...
    public(package) fun remove_liquidity(
        ...
        if (pool_meta.hook_type == HOOK_PREDICTION) {
            let (amounts, burn_position) =
                VeriFiPublisher::tapp_prediction_hook::remove_liquidity(
                    pool_signer, position_idx, stream, creator
                );
            let assets = pool_meta.assets;
            let txs =
    ...
    public(package) fun swap(
        ...
        if (pool_meta.hook_type == HOOK_PREDICTION) {
            let (yes_to_no, amount_in, amount_out) =
                VeriFiPublisher::tapp_prediction_hook::swap(pool_signer, stream, creator);
            let assets = pool_meta.assets;
            return if (yes_to_no) {
                vector[tx(assets[0], amount_in, true), tx(assets[1], amount_out, false)]
            } else {
    ...
    public(package) fun collect_fee(
        ...
        if (pool_meta.hook_type == HOOK_PREDICTION) {
            let (amounts) =
                VeriFiPublisher::tapp_prediction_hook::collect_fee(pool_signer, creator);
            let txs =
                pool_meta.assets.zip_map(
                    amounts, |asset, amount| tx(asset, amount, false)
                );
            return txs
        };
