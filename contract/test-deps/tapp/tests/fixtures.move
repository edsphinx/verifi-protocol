...
module tapp::fixtures {
    ...
    // Prediction Hook Helper Functions

    #[test_only]
    public fun create_prediction_pool(
        signer: &signer, assets: vector<address>, fee: u64
    ): address {
        let pool_args = create_pool_args(4, assets, fee); // HOOK_PREDICTION = 4
        create_pool(signer, pool_args);

        let events = event::emitted_events<PoolCreated>();
        router::event_pool_created_pool_addr(&events[events.length() - 1])
    }

    #[test_only]
    public fun create_prediction_pool_add_liquidity(
        signer: &signer,
        assets: vector<address>,
        fee: u64,
        amount_yes: u64,
        ...
        let args = vector::empty<u8>();
        args.append(to_bytes(&4u8)); // HOOK_PREDICTION = 4
        args.append(to_bytes(&assets));
        args.append(to_bytes(&fee));
        args.append(to_bytes(&amount_yes));
        args.append(to_bytes(&amount_no));
        args.append(to_bytes(&min_lp_tokens));
        router::create_pool_add_liquidity(signer, args);
        let creation_events = event::emitted_events<PoolCreated>();
        router::event_pool_created_pool_addr(&creation_events[creation_events.length() - 1])
    }
    ...
    #[test_only]
    public fun add_liquidity_prediction_pool(
        signer: &signer,
        pool_addr: address,
        position_idx: Option<address>,
        amount_yes: u64,
    ...
    #[test_only]
    public fun remove_liquidity_prediction_pool(
        signer: &signer,
        pool_addr: address,
        position_addr: address,
        lp_tokens: u64,
        ...
        args.append(to_bytes(&pool_addr));
        args.append(to_bytes(&position_addr));
        args.append(to_bytes(&lp_tokens));
        args.append(to_bytes(&min_yes));
        args.append(to_bytes(&min_no));
        router::remove_liquidity(signer, args);
    ...
    #[test_only]
    public fun swap_prediction_pool(
        signer: &signer,
        pool_addr: address,
        amount_in: u64,
        yes_to_no: bool,
        ...
        args.append(to_bytes(&pool_addr));
        args.append(to_bytes(&amount_in));
        args.append(to_bytes(&yes_to_no));
        args.append(to_bytes(&min_amount_out));
        router::swap(signer, args);
    ...
    #[test_only]
    public fun collect_fee_prediction_pool(
        signer: &signer,
        pool_addr: address,
        position_addr: address
    ): vector<u64> {
