...
async function verifyPortfolio() {
  ...
  try {
    ...
    console.log(
      `Total Invested:     $${data.totalInvested?.toFixed(2) || "0.00"}`,
    );
    console.log(
      `Unrealized P&L:     $${data.unrealizedPnL?.toFixed(2) || "0.00"} (${data.unrealizedPnLPct?.toFixed(2) || "0.00"}%)`,
    );
    console.log(
      `Realized P&L:       $${data.realizedPnL?.toFixed(2) || "0.00"}`,
    );
    ...
    if (data.stats) {
      ...
      console.log(
        `Total Volume:       $${data.stats.totalVolume?.toFixed(2) || "0.00"}`,
      );
      console.log(`Winning Trades:     ${data.stats.winningTrades || 0}`);
      console.log(`Losing Trades:      ${data.stats.losingTrades || 0}`);
      console.log(
        `Win Rate:           ${data.stats.winRate?.toFixed(1) || "0.0"}%`,
      );
      console.log(
        `Avg Trade Size:     $${data.stats.avgTradeSize?.toFixed(2) || "0.00"}`,
      );
      console.log("");
    }
    ...
    if (data.openPositions && data.openPositions.length > 0) {
      ...
      data.openPositions.forEach((pos: any, index: number) => {
        ...
        console.log(
          `   Market:          ${pos.marketAddress?.slice(0, 10)}...${pos.marketAddress?.slice(-8)}`,
        );
        console.log(`   Outcome:         ${pos.outcome}`);
        console.log(
          `   Shares Owned:    ${pos.sharesOwned?.toFixed(4) || "0.0000"}`,
        );
        console.log(
          `   Avg Entry Price: $${pos.avgEntryPrice?.toFixed(4) || "0.0000"}`,
        );
        console.log(
          `   Current Price:   $${pos.currentPrice?.toFixed(4) || "0.0000"}`,
        );
        console.log(
          `   Total Invested:  $${pos.totalInvested?.toFixed(2) || "0.00"}`,
        );
        console.log(
          `   Current Value:   $${pos.currentValue?.toFixed(2) || "0.00"}`,
        );
        console.log(
          `   P&L:             $${pos.unrealizedPnL?.toFixed(2) || "0.00"} (${pos.unrealizedPnLPct?.toFixed(2) || "0.00"}%)`,
        );
    ...
    const checks = [
      ...
      },
      {
        ...
        passed:
          ...
          (data.closedPositions?.length || 0) > 0,
      },
    ];
