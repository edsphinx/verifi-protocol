...
export function LiquidityPanel({
  ...
  const { data: userPositions, isLoading: isLoadingPositions } =
    useUserPositions(poolData?.poolAddress || "", account?.address.toString());
                  ...
                  <div className="space-y-2">
                    ...
                    {!validation.addYesError && (
                      <p className="text-xs text-muted-foreground">
                        You'll deposit {yesAmount || "0"} YES +{" "}
                        {noAmount || "0"} NO tokens
                      </p>
                    )}
                  </div>
              ...
              {removePreview && (
                <div className="space-y-2 text-sm bg-muted/50 rounded-lg p-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Share of Pool</span>
                    <span>
                      {formatPercentage(removePreview.shareOfPool, 2)}
                    </span>
                  </div>
                </div>
              )}
